...
import clickerPromises from "../components/ClickerPromises"
...
export default function Clicker() {
    ...
    const [player, setPlayer] = useState({ PlayFabId: "", DisplayName: "New Player", StatValue: 0, Position: "" })
    ...
    const loginAndGetLeaderboard = (async ({ DisplayName, PlayFabId }) => {
        ...
        const playerIndex = newLeaderboard.findIndex((item) => item.PlayFabId === playerId)
        ...
        let newPlayer = { ...player, PlayFabId: playerId, DisplayName: displayName, Position: player.Position + 1 }
        setPlayer(newPlayer)
    ...
    const click = (event) => {
        ...
        clickSound.play()
        soundCounter++
        if (toggleSound === true) { clickSound.playbackRate = 7 }
        if (toggleSound === false) { clickSound.playbackRate = 8 }
        if (soundCounter === 10) { soundCounter = 0; toggleSound = !toggleSound }

        let playerIndex = leaderboard.findIndex((item) => item.PlayFabId === player.PlayFabId)
        leaderboard[playerIndex].StatValue = player.StatValue + 1
        leaderboard.sort((a, b) => b.StatValue - a.StatValue)
        leaderboard.forEach((item, i) => { item.Position = i })
        setLeaderboard(leaderboard)
        playerIndex = leaderboard.findIndex((item) => item.PlayFabId === player.PlayFabId)
            ...
            <div className="clicker-container" onPointerDown={click}>
                <div className="clicker-buttons">
                    <button className="btn"
                        onPointerDown={(event) => { event.preventDefault(); document.body.classList.toggle("clicker-table-hide") }}>
                        Hide Leaderboard
                    </button>
                    <button className="btn"
                        onPointerDown={(event) => { event.preventDefault(); document.body.classList.toggle("clicker-modal-hide") }}>
                        Change Name
                    </button>
                </div>
